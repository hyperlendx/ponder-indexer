...
/**
 * In-memory cache for liquidity indices to avoid redundant database queries
 * 
 * The same liquidity index (asset + timestamp) is often queried multiple times
 * during yield calculations. This cache eliminates redundant queries.
 * 
 * Usage:
 * ```typescript
 * const cache = new LiquidityIndexCache();
 * 
 ...
 * ```
 */
export class LiquidityIndexCache {
    ...
    /**
     * Get liquidity index for an asset at a specific timestamp
    ...
    /**
     * Batch prefetch liquidity indices for known asset/timestamp pairs
